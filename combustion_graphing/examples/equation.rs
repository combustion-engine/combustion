...
fn main() {
...

    {
        ...
        combustion_graphing::graph::line::draw_line_bresenham(0, (h / 2) as i64, w as i64, (h / 2) as i64, &mut plot);
        combustion_graphing::graph::line::draw_line_bresenham((w / 2) as i64, 0, (w / 2) as i64, h as i64, &mut plot);

        let x_domain = -10.0..10.0;
        let y_domain = -10.0..10.0;

        let b = combustion_graphing::bezier::BezierCurve::new(vec![(1.0, 1.0),
                                                                (7.3, 4.4),
                                                                (3.2, 7.4)]);

        combustion_graphing::graph::curve::draw_bezier_curve(w, h, &b, x_domain.clone(), y_domain.clone(), 100, |x0, y0, x1, y1| {
            combustion_graphing::graph::line::draw_line_thick_gaussian(x0, y0, x1, y1, width, hardness, &mut plot);
        });

        combustion_graphing::graph::shape::draw_regular_polygon(w, h, 0.0, -3.0, 3.0, 45.0f64.to_radians(), x_domain.clone(), y_domain.clone(), 3, |x0, y0, x1, y1| {
            combustion_graphing::graph::line::draw_line_thick_gaussian(x0, y0, x1, y1, 10.0, hardness, &mut plot);
        });

        combustion_graphing::graph::shape::draw_circle(250, 400, 50, |x, y, a| {
            combustion_graphing::graph::plot::plot_gaussian_dot(x, y, a, 5.0, hardness, &mut plot);
        });

        combustion_graphing::graph::shape::draw_ellipse(100, 100, 400, 300, |x, y, a| {
            combustion_graphing::graph::plot::plot_gaussian_dot(x, y, a, 5.0, hardness, &mut plot);
        });
        ...
        combustion_graphing::graph::function::graph_linear_equation(w, h, x_domain.clone(), y_domain.clone(), w as usize, func, |x0, y0, x1, y1| {
            combustion_graphing::graph::line::draw_line_thick_gaussian(x0, y0, x1, y1, width, hardness, &mut plot);
        });
