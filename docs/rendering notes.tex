...
You can see this effect more in Figure \ref{fig:lambert_orennayar_comparison} on the next page.
...
\noindent Most of these equations were taken from \href{https://seblagarde.wordpress.com/2013/04/29/memo-on-fresnel-equations/}{Memo on Fresnel equations by Sébastien Lagarde}
...
For metals, the extinction coefficient term ($k$) is important because it effectively represents how conductive the material is.
For Dielectric materials, $k$ is \textbf{always} set to zero. It's also worth noting that we can only trust up to two or so decimal places because of floating point limitations
in computers. They might as well be equal to zero, so by just rounding infinitesimal $k$ values to zero will just improve accuracy overall.
